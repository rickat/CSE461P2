...
public class ShenProxyModify {
	...
	static class Client_handler implements Runnable {
		...
		public ByteBuffer client_to_proxy_to_server() throws IOException {
			...
			String request_line_2 = request_line.substring(0, request_line.length()-8);
			...
			// declare a socket from proxy to server
			Socket proxy_to_server = new Socket(name, port_num);
			assert(proxy_to_server.isConnected());
